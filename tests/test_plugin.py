#!/usr/bin/env python3
"""
Basic test suite for fz-CASMO plugin.
...
def test_with_fz():
    ...
    try:
        ...
        assert "fuel_temp" in variables, "Variable 'fuel_temp' not found in parsed input"
        assert "moderator_temp" in variables, "Variable 'moderator_temp' not found in parsed input"
        print("✓")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(compiled_file, 'r') as f:
                content = f.read()
                assert "4.0" in content or "4" in content, "Variable enrichment not substituted"
...
def main():
    """Run all tests."""
    print("=" * 70)
    print("fz-CASMO Plugin Test Suite")
    print("=" * 70)
    print()
    
    # Change to repository root if needed
    if not os.path.exists(".fz"):
        if os.path.exists("../fz-casmo/.fz"):
            os.chdir("../fz-casmo")
        else:
            print("Error: Could not find .fz directory")
            print("Please run this script from the fz-casmo repository root")
            return 1
